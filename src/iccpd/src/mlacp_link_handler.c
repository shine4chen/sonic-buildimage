...
int mlacp_fsm_arp_set(char *ifname, uint32_t ip, char *mac)
{
    ...
    {
    .n.nlmsg_len = NLMSG_LENGTH(sizeof(struct ndmsg)),.n.nlmsg_flags = NLM_F_REQUEST | NLM_F_CREATE | NLM_F_REPLACE,.n.nlmsg_type =
            RTM_NEWNEIGH,.ndm.ndm_family = AF_INET,.ndm.ndm_state = NUD_REACHABLE,};

    ICCPD_LOG_DEBUG(__FUNCTION__, "Set arp entry for IP:%s  MAC:%s  ifname:%s", show_ip_str(htonl(ip)), mac, ifname);

    if ((sys = system_get_instance()) == NULL)
    {
        ICCPD_LOG_DEBUG(__FUNCTION__, "get sys error.");
        return -1;
    }

    if ((lif = local_if_find_by_name(ifname)) == NULL)
    {
        ICCPD_LOG_DEBUG(__FUNCTION__, "get lif error.");
        return -1;
    }
    req.ndm.ndm_ifindex = lif->ifindex;

    ina.s_addr = htonl(ip);
    if (addattr_l(&req.n, sizeof(req), NDA_DST, (char *)&ina, 4) < 0)
    {
        ICCPD_LOG_DEBUG(__FUNCTION__, "add NDA_DST error.");
        return -1;
    }

    parseMacString(mac, mac_addr);

    if (addattr_l(&req.n, sizeof(req), NDA_LLADDR, mac_addr, 6) < 0)
    {
        ICCPD_LOG_DEBUG(__FUNCTION__, "add NDA_LLADDR error.");
        return -1;
    }

    if (rtnl_talk(&sys->rth, &req.n, NULL, 0) < 0)
    {
        ICCPD_LOG_DEBUG(__FUNCTION__, "rtnl_talk error.");
        return -1;
    }
...
int mlacp_fsm_arp_del(char *ifname, uint32_t ip)
{
    ...
    {
    .n.nlmsg_len = NLMSG_LENGTH(sizeof(struct ndmsg)),.n.nlmsg_flags = NLM_F_REQUEST,.n.nlmsg_type = RTM_DELNEIGH,.ndm.ndm_family =
            AF_INET,.ndm.ndm_state = NUD_REACHABLE,};
...
int mlacp_fsm_ndisc_set(char *ifname, char *ipv6, char *mac)
{
    struct System *sys = NULL;
    struct LocalInterface *lif = NULL;
    uint8_t mac_addr[ETHER_ADDR_LEN];
    ...
    {
    .n.nlmsg_len = NLMSG_LENGTH(sizeof(struct ndmsg)),.n.nlmsg_flags = NLM_F_REQUEST | NLM_F_CREATE | NLM_F_REPLACE,.n.nlmsg_type =
            RTM_NEWNEIGH,.ndm.ndm_family = AF_INET6,.ndm.ndm_state = NUD_REACHABLE,};

    ICCPD_LOG_DEBUG(__FUNCTION__, "Set nd entry for IPv6:%s  MAC:%s  ifname:%s", show_ipv6_str(ipv6), mac, ifname);
    ...
    if (addattr_l(&req.n, sizeof(req), NDA_DST, ipv6, 16) < 0)
    {
        ICCPD_LOG_DEBUG(__FUNCTION__, "add NDA_DST error.");
        return -1;
    }
    ...
    ICCPD_LOG_DEBUG(__FUNCTION__, "set nd entry(%s, %s, %s) by rtnl_talk.", ifname, show_ipv6_str(ipv6), mac);
...
int mlacp_fsm_ndisc_del(char *ifname, char *ipv6)
{
    struct System *sys = NULL;
    struct LocalInterface *lif = NULL;
    ...
    {
    .n.nlmsg_len = NLMSG_LENGTH(sizeof(struct ndmsg)),.n.nlmsg_flags = NLM_F_REQUEST,.n.nlmsg_type = RTM_DELNEIGH,.ndm.ndm_family =
            AF_INET6,.ndm.ndm_state = NUD_REACHABLE,};

    ICCPD_LOG_DEBUG(__FUNCTION__, "Del nd entry for IPv6: %s ifname:%s", show_ip_str(ipv6), ifname);
    ...
    ICCPD_LOG_DEBUG(__FUNCTION__, "del nd entry(%s, %s) by rtnl_talk.", ifname, show_ipv6_str(ipv6));
...
static int arp_set_handler(struct CSM *csm, struct LocalInterface *lif, int add)
{
    struct Msg *msg = NULL;
...
add_arp:
    ...
    TAILQ_FOREACH(msg, &MLACP(csm).arp_list, tail)
    {
        ...
        iccp_netlink_neighbor_request(AF_INET, &arp_msg->ipv4_addr, 1, arp_msg->mac_addr, arp_msg->ifname);
...
del_arp:
    ...
    TAILQ_FOREACH(msg, &MLACP(csm).arp_list, tail)
    {
        ...
        iccp_netlink_neighbor_request(AF_INET, &arp_msg->ipv4_addr, 0, arp_msg->mac_addr, arp_msg->ifname);
 ...
 *
 ****************************************/
static void set_route_by_linux_route(struct CSM *csm, struct LocalInterface *local_if, int is_add)
{
    /* TODO Need to remove this function when set static route with zebra works fine */
...
static void update_vlan_if_info(struct CSM *csm, struct LocalInterface *local_if, struct LocalInterface *vlan_if, int po_state)
{
...
static void update_l3_if_info(struct CSM *csm, struct LocalInterface *local_if, struct LocalInterface *l3_if, int po_state)
{
    if (!csm || !l3_if)
        return;
...
static void update_po_if_info(struct CSM *csm, struct LocalInterface *local_if, int po_state)
{
    if (!csm || !local_if)
        return;
...
static void set_l3_itf_state(struct CSM *csm, struct LocalInterface *set_l3_local_if, ROUTE_MANIPULATE_TYPE_E route_type)
{
...
void set_peerlink_mlag_port_learn(struct LocalInterface *lif, int enable)
{
    ...
    ICCPD_LOG_DEBUG(__FUNCTION__, " send port-learn msg to sync for %s , member %s", lif->name, sub_msg->data);
...
static void set_peerlink_mlag_port_kernel_forward(struct CSM *csm, struct LocalInterface *lif, int enable)
{
    ...
    sprintf(cmd, "ebtables %s FORWARD -i %s -o %s -j DROP", "-D", csm->peer_link_if->name, lif->name);
    ICCPD_LOG_DEBUG(__FUNCTION__, " ebtable cmd  %s", cmd);
    system(cmd);

    sprintf(cmd, "ebtables %s FORWARD -i %s -o %s -j DROP", (enable) ? "-A" : "-D", csm->peer_link_if->name, lif->name);
    ICCPD_LOG_DEBUG(__FUNCTION__, " ebtable cmd  %s", cmd);
    system(cmd);
...
static void set_peerlink_mlag_port_isolate(struct CSM *csm, struct LocalInterface *lif, int enable)
{
    if (!lif)
        return;
    ...
    ICCPD_LOG_DEBUG(__FUNCTION__, "%s  port-isolate from %s to %s", enable ? "Enable" : "Disable", csm->peer_link_if->name, lif->name);
...
void update_peerlink_isolate_from_pif(struct CSM *csm, struct PeerInterface *pif, int pif_po_state, int new_create)
{
    struct LocalInterface *lif = NULL;
    int lif_po_state = 1;
    ...
    ICCPD_LOG_DEBUG(__FUNCTION__, " from peer %s local(%s) / peer(%s)", lif->name, (lif_po_state) ? "up" : "down", (pif_po_state) ? "up" : "down");
    ...
    if (lif_po_state == 1)
    {
        if (pif_po_state == 1)
        {
            ...
            ICCPD_LOG_DEBUG(__FUNCTION__, " Enable port-isolate from %s to %s", csm->peer_link_if->name, lif->name);
            set_peerlink_mlag_port_isolate(csm, lif, 1);
        }
        else
        {
            /* local link up, and peer link changes to down, disable port-isolate */
            ICCPD_LOG_DEBUG(__FUNCTION__, "Disable port-isolate from %s to %s", csm->peer_link_if->name, lif->name);
...
static void update_peerlink_isolate_from_lif(struct CSM *csm, struct LocalInterface *lif, int lif_po_state)
{
    struct PeerInterface *pif = NULL;
    int pif_po_state = 1;
    ...
    ICCPD_LOG_DEBUG(__FUNCTION__, " from local %s local(%s) / peer(%s)", lif->name, (lif_po_state) ? "up" : "down", (pif_po_state) ? "up" : "down");
...
static void update_l2_po_state(struct CSM *csm, struct LocalInterface *lif, int po_state)
{
    ...
    /* L2 po */
    /* if (po_state != lif->po_active && po_state == 0) { mlacp_clean_fdb(); } */
    ...
    LIST_FOREACH(vlan, &(lif->vlan_list), port_next)
    {
        ...
        /* NOTE assume only one mlag per vlan need to add rules for per mlag per vlan later (arp list?) */
        set_l3_vlan_if = vlan->vlan_itf;
        if (po_state != lif->po_active || MLACP(csm).current_state != set_l3_vlan_if->mlacp_state)
        {
            if (po_state == 1)
            {
                route_type = ROUTE_DEL;
            }
            else if (po_state == 0 && MLACP(csm).current_state == MLACP_STATE_EXCHANGE)
            {
...
static void update_l3_po_state(struct CSM *csm, struct LocalInterface *lif, int po_state)
{
    ROUTE_MANIPULATE_TYPE_E route_type = ROUTE_NONE;
    ...
    if (po_state != lif->po_active && po_state == 1)
    {
        /* po alive, clean static route & recover the ARP */
        route_type = ROUTE_DEL;
    }
    else if (po_state != lif->po_active && po_state == 0 && MLACP(csm).current_state == MLACP_STATE_EXCHANGE)
    {
        /* po is not alive & peer-link alive, set static route */
        if (peer_po_is_alive(csm, lif->po_id) == 1)
            route_type = ROUTE_ADD;
    }
    else if (MLACP(csm).current_state != lif->mlacp_state && MLACP(csm).current_state == MLACP_STATE_EXCHANGE && po_state == 0)
    {
    ...
    if (!TAILQ_EMPTY(&(MLACP(csm).arp_list)))
    {
        TAILQ_FOREACH(msg, &MLACP(csm).arp_list, tail)
        {
            ...
            if (iccp_csm_init_msg(&msg_send, (char *)arp_msg, sizeof(struct ARPMsg)) == 0)
            {
                ...
                ICCPD_LOG_DEBUG(__FUNCTION__, "Failed to enqueue ARP[ADD] for %s", show_ip_str(arp_msg->ipv4_addr));
        }
    }
...
void update_stp_peer_link(struct CSM *csm, struct PeerInterface *pif, int po_state, int new_create)
{
    struct LocalInterface *lif = NULL;
    struct VLAN_ID *vlan = NULL;
    ...
    LIST_FOREACH(lif, &(MLACP(csm).lif_list), mlacp_next)
    {
        ...
        else
        {
            LIST_FOREACH(vlan, &(lif->vlan_list), port_next)
            {
                ...
                /* NOTE assume only one mlag per bridge need to add rules for per mlag per bridge later (arp list?) */
                if (po_state == 1 && lif->po_active == 0)
...
void iccp_get_fdb_change_from_syncd(void)
{
    ...
    ICCPD_LOG_DEBUG(__FUNCTION__, "fd %d write get fdb change msg ", sys->sync_fd);
...
void iccp_send_fdb_entry_to_syncd(struct MACMsg *mac_msg, uint8_t mac_type)
{
    ...
    ICCPD_LOG_DEBUG(__FUNCTION__, "write mac msg vid : %d ; ifname %s ; mac %s fdb type %s ; op type %s",
                    ...
                    mac_info->op_type == MAC_SYNC_ADD ? "add" : "del");
...
static void update_l2_mac_state(struct CSM *csm, struct LocalInterface *lif, int po_state)
{
    struct Msg *msg = NULL;
    ...
    TAILQ_FOREACH(msg, &MLACP(csm).mac_list, tail)
    {
        ...
        if (po_state == 0)
        {
            ...
            if (mac_msg->age_flag == (MAC_AGE_LOCAL | MAC_AGE_PEER))
            {
                ...
                ICCPD_LOG_DEBUG(__FUNCTION__, "Intf down, del MAC: %s, MAC %s vlan-id %d", mac_msg->ifname, mac_msg->mac_str, mac_msg->vid);
        ...
        else    /* portchannel up */
        {
            ...
            else
            {
                ...
                ICCPD_LOG_DEBUG(__FUNCTION__, "Intf up, add MAC to ASIC: %s, MAC %s vlan-id %d", mac_msg->ifname, mac_msg->mac_str, mac_msg->vid);
...
static void mlacp_conn_handler_fdb(struct CSM *csm)
{
    ...
    if (!TAILQ_EMPTY(&(MLACP(csm).mac_list)))
    {
        TAILQ_FOREACH(msg, &MLACP(csm).mac_list, tail)
        {
            ...
            /* If MAC with local age flag, dont sync to peer. Such MAC only exist when peer is warm-reboot. If peer is warm-reboot, peer age flag is
               not set when connection is lost. When MAC is aged in local switch, this MAC is not deleted for no peer age flag. After warm-reboot,
               this MAC must be learnt by peer and sync to local switch */
            ...
            else
            {
                ...
                if (strcmp(mac_msg->ifname, csm->peer_itf_name) != 0)
                {
                    ICCPD_LOG_DEBUG(__FUNCTION__,
                                    "MAC-msg-list not enqueue for local age flag: %s, mac %s vlan-id %d, age_flag %d, remove local age flag",
                                    mac_msg->ifname, mac_msg->mac_str, mac_msg->vid, mac_msg->age_flag);
...
void mlacp_peer_disconn_handler(struct CSM *csm)
{
    ...
    if (csm->peer_warm_reboot_time != 0)
    {
        ...
        /* peer connection must be establised again within 90s from last disconnection for peer warm reboot */
...
conn_fail:
    if (count == 0)
        ICCPD_LOG_DEBUG(__FUNCTION__, "%s:%d, mclag syncd socket connect fail", __FUNCTION__, __LINE__);
...
void do_mac_update_from_syncd(char mac_str[ETHER_ADDR_STR_LEN], uint16_t vid, char *ifname, uint8_t fdb_type, uint8_t op_type)
{
    ...
    if (op_type == MAC_SYNC_ADD)
    {
        ...
        if (mac_exist)
        {
            ...
            /* if(strcmp(csm->peer_itf_name, mac_info->ifname) == 0) { return; } */

            /* update MAC */
            if (mac_info->fdb_type != mac_msg->fdb_type
                || strcmp(mac_info->ifname, mac_msg->ifname) != 0 || strcmp(mac_info->origin_ifname, mac_msg->ifname) != 0)
            {
        ...
        {
            /* If the port the mac learn is change to down before the mac sync to iccp, this mac must be deleted */
            ...
            mac_msg->age_flag |= MAC_AGE_PEER;
            /* ICCPD_LOG_DEBUG(__FUNCTION__, "Add peer age flag: %s, add %s vlan-id %d, age_flag %d", mac_msg->ifname, mac_msg->mac_str,
               mac_msg->vid, mac_msg->age_flag); */
            mac_msg->op_type = MAC_SYNC_ADD;
            ...
            if (MLACP(csm).current_state == MLACP_STATE_EXCHANGE)
            {
                ...
                if (iccp_csm_init_msg(&msg_send, (char *)mac_msg, msg_len) == 0)
                {
                    ...
                    /* ICCPD_LOG_DEBUG(__FUNCTION__, "MAC-msg-list enqueue: %s, add %s vlan-id %d, age_flag %d", mac_msg->ifname, mac_msg->mac_str,
                       mac_msg->vid, mac_msg->age_flag); */
                }
                else
                {
            ...
            if (iccp_csm_init_msg(&msg, (char *)mac_msg, msg_len) == 0)
            {
                ...
                /* ICCPD_LOG_DEBUG(__FUNCTION__, "MAC-list enqueue: %s, add %s vlan-id %d", mac_msg->ifname, mac_msg->mac_str, mac_msg->vid); */
            }
            else
                ICCPD_LOG_DEBUG(__FUNCTION__, "Failed to enqueue MAC %s, add %s vlan-id %d", mac_msg->ifname, mac_msg->mac_str, mac_msg->vid);
        }
    }
    else        /* handle mac del */
    {
        ...
        if (mac_exist)
        {
            ...
            if (mac_info->age_flag == (MAC_AGE_LOCAL | MAC_AGE_PEER))
            {
                ICCPD_LOG_DEBUG(__FUNCTION__, "Recv MAC del msg: %s, del %s vlan-id %d", mac_info->ifname, mac_info->mac_str, mac_info->vid);
...
int iccp_receive_fdb_handler_from_syncd(struct System *sys)
{
    ...
    while (pos < n)
    {
        ...
        for (i = 0; i < count; i++)
        {
            ...
            /* ICCPD_LOG_DEBUG(__FUNCTION__, "recv msg fdb count %d vid %d mac %s port %s optype %s ", i, mac_info->vid, mac_info->mac,
               mac_info->port_name, mac_info->op_type == MAC_SYNC_ADD ? "add" : "del"); */
